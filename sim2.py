...
#find rows and columns (has some extra computations that arent necessary)
    for i in range(0,len(x_k)):
        a_x.append(find_nearest(LUT[:,0], x_k[i]))
        a_y.append(find_nearest(LUT[:,1], y_k[i]))
    ...
    loc = 0
    cols = 0
    for i in range(0,len(aout_prev)):
        if((i + 2) < len(aout_prev)):
            # if(aout[i] == 162) & (aout[i+1] == 29) & (aout[i+2] == 92): #UW found
            if(aout_prev[i] == uw[0]) & (aout_prev[i+1] == uw[1]) & (aout_prev[i+2] == uw[2]):
                if flag == 0:
                    UW = aout_prev[i:i+uwlen]
                if flag == 1:
                    res = loc
                    rows = i - loc
                    res = res - rows - uwlen #find residual data
                loc = i + uwlen
                if (loc < len(aout_prev)):
                    i = loc
                flag = flag + 1
                cols = cols + 1
    cols = cols - 1
...
#decision output
    ...
    plt.figure()
    plt.plot(e)
    plt.title("Sim2_2022 Error Signal"); plt.xlabel("Symbols index");plt.ylabel("Error")
    plt.figure()
    plt.plot(theta_out)
    plt.title("Sim2_2022 Phase Estimate"); plt.xlabel("Symbols index");plt.ylabel("Phase")
    plt.figure()
    plt.imshow(255-aout,cmap=plt.get_cmap('Greys'))
    plt.title("sim2_2022 Results")
